...
| [workbench.md](workbench.md) | the cross-session attention inbox: `WorkbenchSnapshot`, B-class batch confirms with per-item outcomes, C-class decisions, the `workbench/attention-updated` push |
| [task-flow.md](task-flow.md) | the task-flow workbench domain: recipe identity/revisions, task projections, the journal, deliverable versions |
