# Task-flow workbench domain

English | [中文](task-flow.zh.md)

The cross-session task-flow domain: immutable recipe revisions, task/run/phase projections, the append-only workbench journal, deliverable versions, and the recipe engine that schedules phase runs and drives the submission-gate-pass chain. The [recipe registry](../../packages/task-flow/recipe/README.md), the task service, and the [recipe engine](../../packages/task-flow/recipe-engine-core/README.md) own their method contracts; this page records the wire types from [`packages/task-flow/recipe/src/types.ts`](../../packages/task-flow/recipe/src/types.ts) as they land.
