...
export class GateService extends Service {
   ...
   */
  private async maybeAwaitDecision(phaseRun: PhaseRunRecord): Promise<void> {
    try {
      ...
      const complexChecks = pinned.payload.gateChecks.filter(check => (
        check.phaseId === phaseRun.phaseId && check.kind !== 'A'
      ))
