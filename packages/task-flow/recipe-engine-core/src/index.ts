...
import { RecipeError } from '@deepseek-ai/dsh-recipe'
import type {
  RecipeGateCheckSpec,
  RecipePhaseSpec,
  RecipeRevision,
...
import { recipeEngineDomainSpec } from './spec.ts'
import {
  RecipeEngineError,
  ...
  type PhaseOutcome,
  type PhaseSession,
  type PhaseSessionBinding,
} from './types.ts'

/** Task states that end a task; the engine stops scheduling them. */
const TERMINAL_TASK_STATES: readonly TaskState[] = ['completed', 'failed', 'cancelled']

/** Phase-run states that end a phase; the engine never re-executes them. */
const TERMINAL_PHASE_STATES: readonly PhaseRunState[] = ['passed', 'failed', 'cancelled', 'superseded', 'stale']
...
export class RecipeEngineCore extends Service {
  ...
  private executor: PhaseExecutor | undefined
  private bindingsTable: KvTable<string, PhaseSessionBinding> | undefined
  ...
  private readonly inFlight = new Map<string, { taskKey: string; promise: Promise<unknown> }>()
  ...
  registerExecutor(executor: PhaseExecutor): () => void {
    if (this.executor !== undefined) throw new Error('a phase executor is already registered')
  ...
  async trigger(taskId: TaskId): Promise<void> {
    if (this.bindingsTable === undefined) return
  ...
  private requireBindings(): KvTable<string, PhaseSessionBinding> {
    if (this.bindingsTable === undefined) throw new Error('recipe-engine bindings table is not open')
   ...
   * rebuild.
   */
  private validateJournalHead(task: TaskRecord): void {
    const facts = this.ctx.workbenchJournal.replay(0).filter(
      fact => fact.taskId === task.taskId && fact.kind === 'task/updated',
    )
  ...
  private poison(taskId: TaskId, message: string): void {
    const key = String(taskId)
   ...
   */
  private async scheduleNow(taskId: TaskId): Promise<void> {
    const key = String(taskId)
    if (this.poisoned.has(key)) return
  ...
  private resolvePinned(task: TaskRecord): RecipeRevision | undefined {
    let pinned: RecipeRevision
    try {
   ...
   */
  private async advanceTask(task: TaskRecord, pinned: RecipeRevision): Promise<boolean> {
    ...
    for (const run of runs) {
      if (TERMINAL_PHASE_STATES.includes(run.state)) this.disposeSession(String(run.phaseRunId))
    }
    ...
    if (active.length > 1) {
      ...
      for (const run of active) {
        if (keep === undefined || run.phaseRunId === keep.phaseRunId) continue
  ...
  private async executePhase(
    ...
    phase: RecipePhaseSpec,
    pinned: RecipeRevision,
  ...
  private submissionIdFor(phaseRun: PhaseRunRecord, attempt: number): SubmissionId {
  ...
  private async cancelActivePhases(task: TaskRecord): Promise<void> {
    const runId = task.currentRunId
  ...
  private hasInFlightForTask(taskKey: string): boolean {
    for (const entry of this.inFlight.values()) {
      if (entry.taskKey === taskKey) return true
    }
    return false
  }

  private bindingOf(phaseRunKey: string): PhaseSessionBinding | undefined {
    return this.requireBindings().get(phaseRunKey)
  }

  private async putBinding(binding: PhaseSessionBinding): Promise<void> {
    await this.requireBindings().put(binding.phaseRunId, binding)
  }

  private disposeSession(phaseRunKey: string): void {
  ...
  private disposeTaskSessions(task: TaskRecord): void {
    for (const [, binding] of this.requireBindings().entries()) {
      if (binding.taskId === task.taskId) this.disposeSession(String(binding.phaseRunId))
    }
  }

  private requireExecutor(): PhaseExecutor {
