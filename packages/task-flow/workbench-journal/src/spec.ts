...
import type { JournalEventId, JournalFact, JournalPayload } from './types.ts'

/** JSON value accepted as a fact payload at the durable boundary. */
const jsonValue: z.ZodType<JournalPayload> = z.lazy(() =>
...
const journalEventId = z.string().min(1).transform(value => value as JournalEventId)
...
}) as unknown as z.ZodType<JournalFact>
 ...
 */
export function journalSeqKey(journalSeq: number): string {
...
export const workbenchJournalDomainSpec = defineDomain({
  ...
  version: 1,
  tables: {
    entries: domainTable<string, JournalFact>(journalFactSchema),
  },
})
