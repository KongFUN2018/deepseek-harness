...
import type { DeliverableVersionRef, PhaseRunId, SubmissionId, TaskId } from '@deepseek-ai/dsh-task/types'
import type { JournalFact, JournalPayload } from '@deepseek-ai/dsh-workbench-journal/types'
import '@deepseek-ai/dsh-workbench-journal'
import { DeliverableId, DeliverableVersionId as DeliverableVersionIdValue, ImpactSnapshotId, UNTASKED_FACT_TASK_ID } from './runtime.ts'
import { deliverableLocalDomainSpec } from './spec.ts'
import type { LatestVersionEntry, PhaseInputs, SaveKeyEntry } from './spec.ts'
...
import type {
  ...
  ImpactSnapshot,
  ImpactStaledGateChecks,
  ImpactStaledVersions,
} from './types.ts'
...
/** One save request after wire validation, branded where the record brands. */
interface SaveInput {
  readonly deliverableId: ReturnType<typeof DeliverableId>
  readonly expectedBaseVersion: DeliverableVersionId | null
  readonly sourceSubmissionId: SubmissionId | undefined
  readonly idempotencyKey: string | undefined
}

/** The journal fact key of one save's commit point. */
function saveFactKey(idempotencyKey: string): string {
...
export class DeliverableService extends TypertRemoteService {
  ...
  static inject = ['storageDomain', 'workbenchJournal']

  private versions?: KvTable<string, DeliverableVersion>
  private phaseInputs?: KvTable<string, PhaseInputs>
  private saveKeys?: KvTable<string, SaveKeyEntry>
  private latest?: KvTable<string, LatestVersionEntry>
  private snapshots?: KvTable<string, ImpactSnapshot>
  ...
  listConsumingPhaseRuns(targetVersionId: string): PhaseRunId[] {
    ...
    const consuming: PhaseRunId[] = []
    for (const [runId, inputs] of this.requirePhaseInputs().entries()) {
      if (inputs.inputVersionIds.includes(target)) consuming.push(runId as PhaseRunId)
    }
    return consuming
  }
  ...
  private async saveVersionNow(input: SaveInput): Promise<DeliverableVersion> {
   ...
   */
  private async replaySave(input: SaveInput): Promise<DeliverableVersion | undefined> {
    const indexed = this.requireSaveKeys().get(input.idempotencyKey as string)
    if (indexed !== undefined) {
      const stored = this.requireVersions().get(indexed.versionId)
      if (stored === undefined) {
        throw new DeliverableError('not-found', `save index entry "${input.idempotencyKey as string}" names a missing version`)
      }
      this.assertSaveFieldsMatch(input, stored)
      return stored
    }
    const fact = this.factByKey(saveFactKey(input.idempotencyKey as string))
    if (fact === undefined) return undefined
    const recovered = fact.payload as unknown as DeliverableVersion
    ...
    await this.requireSaveKeys().put(input.idempotencyKey as string, { versionId: recovered.versionId })
    return recovered
  }

  /** Compare a replayed save's caller fields with the stored version; any difference is a conflict. */
  private assertSaveFieldsMatch(input: SaveInput, stored: DeliverableVersion): void {
    ...
    if (!same) {
      throw new DeliverableError('idempotency-conflict', `idempotency key "${input.idempotencyKey as string}" was already used with different fields`)
    }
  }

  /** One serialized dependency registration; the durable put completes the version record. */
  private async registerVersionDependenciesNow(
    versionId: DeliverableVersionId,
    dependsOn: readonly DeliverableVersionRef[],
  ): Promise<void> {
    const versions = this.requireVersions()
    const version = versions.get(versionId)
    ...
    const completed: DeliverableVersion = { ...version, dependsOn }
   ...
   */
  private async invalidateDownstreamNow(roots: DeliverableVersionId[]): Promise<ImpactSnapshot> {
    const versions = this.requireVersions()
    ...
    const consumers = new Map<string, DeliverableVersion[]>()
    ...
    const staled = new Map<string, DeliverableVersion>()
    const queue = [...rootVersions]
    const seen = new Set<string>(roots.map(String))
    while (queue.length > 0) {
      const version = queue.shift() as DeliverableVersion
  ...
  private groupStaledVersions(staled: ReadonlyMap<string, DeliverableVersion>): ImpactStaledVersions[] {
  ...
  private affectedRuns(staled: ReadonlyMap<string, DeliverableVersion>): PhaseRunId[] {
    const affected: PhaseRunId[] = []
    for (const [runId, inputs] of this.requirePhaseInputs().entries()) {
      if (inputs.inputVersionIds.some((id: string) => staled.has(id))) affected.push(runId as PhaseRunId)
    }
    return affected
  }
   ...
   */
  private coveredGateChecks(affectedPhaseRuns: readonly PhaseRunId[]): ImpactStaledGateChecks[] {
    if (affectedPhaseRuns.length === 0) return []
    const runs = new Set<string>(affectedPhaseRuns.map(String))
    const submissions = new Set<string>()
    for (const fact of this.ctx.workbenchJournal.replay(0)) {
      if (fact.kind !== 'submission/recorded') continue
      const payload = fact.payload as { phaseRunId?: unknown; submissionId?: unknown }
    ...
    for (const fact of this.ctx.workbenchJournal.replay(0)) {
      ...
      const payload = fact.payload as { submissionId?: unknown; checkId?: unknown }
   ...
   */
  private factTaskOf(version: DeliverableVersion): TaskId {
    if (version.sourceSubmissionId === undefined) return UNTASKED_FACT_TASK_ID
    const fact = this.factsOfKind('submission/recorded')
      .find(candidate => (candidate.payload as { submissionId?: unknown }).submissionId === String(version.sourceSubmissionId))
    if (fact === undefined) return UNTASKED_FACT_TASK_ID
    return fact.taskId
  }

  /** Every stored fact of one kind, in journal order. */
  private factsOfKind(kind: string): JournalFact[] {
  ...
  private factByKey(key: string): JournalFact | undefined {
  ...
  private async appendFact(input: {
    readonly kind: DeliverableFactKind
  ...
  private requireVersions(): KvTable<string, DeliverableVersion> {
    if (this.versions === undefined) throw new DeliverableError('invalid-argument', 'deliverable domain is not open')
    return this.versions
  }

  private requirePhaseInputs(): KvTable<string, PhaseInputs> {
    if (this.phaseInputs === undefined) throw new DeliverableError('invalid-argument', 'deliverable domain is not open')
    return this.phaseInputs
  }

  private requireSaveKeys(): KvTable<string, SaveKeyEntry> {
    if (this.saveKeys === undefined) throw new DeliverableError('invalid-argument', 'deliverable domain is not open')
    return this.saveKeys
  }

  private requireLatest(): KvTable<string, LatestVersionEntry> {
    if (this.latest === undefined) throw new DeliverableError('invalid-argument', 'deliverable domain is not open')
    return this.latest
  }

  private requireSnapshots(): KvTable<string, ImpactSnapshot> {
    if (this.snapshots === undefined) throw new DeliverableError('invalid-argument', 'deliverable domain is not open')
